...
func TestNewPathOfBuilding(t *testing.T) {
	result, err := NewPathOfBuilding()
	require.NoError(t, err)
	require.NotZero(t, result)
}
...
func TestTODOREmove(t *testing.T) {
	...
	require.NoError(t, err)
	// String-level processing prevents these from
...
func TestDecodeEncode(t *testing.T) {
	...
	result.Build.Bandit = "Alira"

	var outBuf bytes.Buffer
	err = EncodePOBCode(result, &outBuf)
	require.NoError(t, err)

	_, err = DecodePOBCode(&outBuf)
	require.NoError(t, err)

	// Normally, we'd test to ensure we got the same output as we
